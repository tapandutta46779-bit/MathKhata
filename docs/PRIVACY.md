...
- AION is optional. In the packaged desktop app and local development it can use the configured loopback service on `127.0.0.1`. On the public website, Qwen3 8B runs on the user's device through WebLLM and WebGPU. Its model files are fetched from the published MLC/Hugging Face distribution and cached by the browser; notebook content and questions are not sent to a hosted inference service.
...
This beta does not include a user account, server-side notebook retention, or individual profiling. Cloudflare Web Analytics does not receive notebook content, equations, or microphone recordings. Feedback submitted through an external issue tracker is governed by that service's privacy terms and should not contain private notebook material.
