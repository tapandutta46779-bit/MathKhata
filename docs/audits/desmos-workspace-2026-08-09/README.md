...
7. Geometry and 2D have continuous major/minor grids and pointer-centered wheel zoom. 2D and Geometry support pan, zoom in/out, home/reset, and viewport locking.
8. 3D has a clearer expression rail plus an on-canvas settings panel for rendering, projection, axes, axis numbers, XY grid, bounding cube, intersections, camera locks, perspective, independent x/y/z bounds, mesh density, and camera presets. Horizontal orbit now follows Desmos direction and uses a calmer calibrated sensitivity. The 3D canvas supports orbit, wheel zoom, trace, surfaces, points, curves, parameters, sampled implicit surfaces, and sampled intersections.
...
- 12 end-to-end tests cover the full notebook flow, MathLive keyboard/menu/symbol access, voice, AION, long structured mathematics, 2D/3D/Geometry/Scientific workflows, circle radius editing, independent 3D bounds, Desmos-direction orbit and pointer release, settings dismissal, square-coordinate canvas sizing, and viewport containment.
