...
Deterministic page analysis remains the reliable foundation. In the packaged desktop app or local development, the assistant can optionally use the published Qwen3 8B Q4_K_M model through Ollama. This **Local Qwen Assistant** can answer free-form questions and present visible solution steps; deterministic grouping and CAS remain independent verification layers. The public website does not call localhost or a hosted model. MathKhata has not trained Qwen3, and this integration is not the author's future original non-LLM AION architecture. Details are in [docs/LOCAL_MODEL_PATH.md](docs/LOCAL_MODEL_PATH.md).
