...
export interface AIONAnswer {
  text: string;
  model: string;
  runtime: 'ollama' | 'webgpu' | 'cloudflare';
}
...
export async function checkAIONOnline(signal?: AbortSignal): Promise<AIONLocalStatus> {
  try {
    ...
    return {
      ...
      model: 'AION online',
      message: ready
        ? 'AION online is ready. It starts quickly and works on mobile.'
        : payload.message || payload.error || 'AION online is temporarily unavailable.',
    };
  } catch (error) {
    const reason = error instanceof Error ? error.message : 'the online service could not be reached';
    return {
      reachable: false,
      modelReady: false,
      model: 'AION online',
      message: `AION online is unavailable: ${reason}.`,
    };
  }
}

export async function askAIONOnline(
  prompt: string,
  options: {
    signal?: AbortSignal;
  ...
  for (let segment = 0; segment < 3; segment += 1) {
    options.onStatus?.(segment === 0 ? 'AION online is working through the mathematics…' : 'AION online is completing the remaining steps…');
    const response = await fetch(AION_PUBLIC_ENDPOINT, {
      method: 'POST',
    ...
    if (!response.ok) {
      const payload = await response.json().catch(() => ({})) as PublicAIONStatusResponse;
      throw new Error(payload.error || payload.message || `AION online returned ${response.status}.`);
    }
    if (!response.body) throw new Error('AION online returned no response stream.');
    ...
    requestPrompt = [
      prompt,
      '',
      'Continue exactly where the previous visible answer stopped.',
      'Do not repeat earlier material. Finish the remaining steps, verification, and final result.',
      '',
      'Previous visible answer:',
      cleanVisibleAnswer(answerText),
    ].join('\n');
  }
  if (!completed) throw new Error('AION online could not finish this unusually long answer.');
  const text = cleanVisibleAnswer(answerText);
  if (!text) throw new Error('AION online returned an empty response.');
  return { text, model: 'AION', runtime: 'cloudflare' };
}
...
export async function checkAIONLocal(signal?: AbortSignal): Promise<AIONLocalStatus> {
  try {
    ...
    if (desktop) {
      ...
      const ready = window.isSecureContext && 'gpu' in navigator;
      return {
        reachable: ready,
        modelReady: ready,
        model: 'AION',
        message: ready
          ? 'AION is ready. Its on-device model downloads and caches when you first ask a question.'
          : 'AION requires WebGPU in a current secure browser and enough device memory.',
      };
    }
...
export async function askAIONLocal(
  ...
  if (desktop) {
    ...
    const { askAIONWebGPU } = await import('./webgpuProvider');
    answerText = await askAIONWebGPU(prompt, options);
  }
  ...
  return {
    ...
    runtime: import.meta.env.DEV || window.mathKhataDesktop?.aion ? 'ollama' : 'webgpu',
  };
}
...
export async function askAIONAboutPage(
  ...
  onUpdate?: (text: string) => void,
  onStatus?: (message: string, progress?: number) => void,
  provider: 'on-device' | 'online' = 'on-device',
): Promise<AIONAnswer> {
  ...
  return provider === 'online'
    ? askAIONOnline(prompt, { signal, onUpdate, onStatus })
    : askAIONLocal(prompt, { signal, onUpdate, onStatus });
}
